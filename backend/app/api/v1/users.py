...
async def create_user_profile_cv(
    ...
    """
    logger.debug(f"DEBUG: Starting create_user_profile_cv for user_id: {user_id}")

    # 1) Check if a profile already exists for this user
    logger.debug(f"DEBUG: Checking for existing profile for user_id: {user_id}")
    existing_profile = (
    ...
    logger.debug(f"DEBUG: Existing profile data: {existing_profile.data}")

    if existing_profile.data and existing_profile.data[0].get("full_name"):
        logger.debug("DEBUG: Profile already exists, raising 409.")
    ...
    try:
        ...
        logger.debug("DEBUG: Preparing profile payload.")
        ...
        logger.debug(f"DEBUG: Profile payload: {profile_payload}")

        if existing_profile.data:
            logger.debug("DEBUG: Updating existing profile in 'profiles' table.")
            profile_response = (
                supabase.table("profiles")
            ...
            logger.debug("DEBUG: Inserting new profile into 'profiles' table.")
            profile_payload["created_at"] = now_iso
            profile_response = (
                supabase.table("profiles").insert(profile_payload).execute()
            )
        logger.debug(f"DEBUG: Profile Supabase response data: {profile_response.data}")

        if not profile_response.data:
            logger.debug("DEBUG: Profile response data is empty, raising 500.")
            raise HTTPException(
        ...
        logger.debug(f"DEBUG: Profile record from Supabase: {profile_record}")

        # 3) Insert a CV row in public.cv_documents
        logger.debug("DEBUG: Preparing CV payload.")
        cv_payload = {
            "user_id": user_id,
        ...
        logger.debug(f"DEBUG: CV payload: {cv_payload}")

        cv_insert_response = (
            supabase.table("cv_documents").insert(cv_payload).execute()
        )
        logger.debug(f"DEBUG: CV Supabase response data: {cv_insert_response.data}")

        if not cv_insert_response.data:
            logger.debug("DEBUG: CV response data is empty, raising 500.")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.debug(f"DEBUG: UserProfileResponse created: {response_data}")
        return response_data

    except HTTPException:
        logger.debug("DEBUG: Caught HTTPException, re-raising.")
        raise
    except Exception as e:
        logger.debug(f"DEBUG: Caught generic Exception: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_user_profile_cv(
    ...
    Handles partial updates of profile fields and cv_content.
    """
    ...
    if profile_cv_update_data.cv_content is not None:
        if not profile_cv_update_data.cv_content.strip():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="'cv_content' cannot be empty.",
            )
        cv_update_payload["cv_content"] = profile_cv_update_data.cv_content
    ...
    try:
        ...
        cv_response = (
            supabase.table("cv_documents")
            .select("cv_content, created_at, updated_at")
            .eq("user_id", user_id)
            .limit(1)
            .execute()
        )
        ...
        return UserProfileResponse(
            ...
            address=profile_record.get("address", ""),
            cv_content=cv_record.get("cv_content", ""),
